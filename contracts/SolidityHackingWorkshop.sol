...
//*** Solution Exercise 1***//
// function sendToken only checks whether sender has a != 0 amount of tokens
// a sender can send more tokens than he actually has
// fixed in commented line 29
...
contract VoteTwoChoices{
    ...
    function vote(uint _nbVotes, bytes32 _proposition) {
        //require(_nbVotes <= votingRights[msg.sender]);
...
//*** Solution Exercise 2***//
// vote function is vulnerable to uint overflow with _nbVotes paramenter.
// we can make (_nbVotes + votesCast[msg.sender] to be <= as votingRights
// we cast some votes in a first attempt.
// In the second, we vote again by making _nbVotes = uint256 max - votesCast
// it will pass the requirement since _nbVotes + votesCast[msg.sender] == 0
// fixed in line 62
